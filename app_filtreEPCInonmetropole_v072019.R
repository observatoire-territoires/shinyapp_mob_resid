...
indics_migres_TERR <- readRDS("indics_migres_TERR.rds") %>% ungroup() #%>% mutate(LIB_TERR = iconv(LIB_TERR, "latin1", "UTF-8"))
indics_migres_TERR_CS1 <- read_rds("indics_migres_TERR_CS1.rds") %>% ungroup() #%>% mutate(LIB_TERR = iconv(LIB_TERR, "latin1", "UTF-8"))
indics_migres_TERR_AGEREVS <- read_rds("indics_migres_TERR_AGEREVS.rds" ) %>% ungroup() #%>% mutate(LIB_TERR = iconv(LIB_TERR, "latin1", "UTF-8"))

# indicateurs de renouvellement
indics_migres_TERR_AGEREVS_RENOUV <- read_rds("indics_migres_TERR_AGEREVS_RENOUV.rds" ) %>% ungroup() 
indics_migres_TERR_CS1_RENOUV <- read_rds("indics_migres_TERR_CS1_RENOUV.rds" ) %>% ungroup() #%>% mutate(LIB_TERR = iconv(LIB_TERR, "latin1", "UTF-8"))

# volume des flux
flux_migres_TERR.map <- read_rds("flux_migres_TERR.map.rds" ) %>% ungroup() 
flux_migres_TERR.map.SMnet <- read_rds("flux_migres_TERR.map.SMnet.rds" ) %>% ungroup() 

# indics histodemo
indics_TERR_histodemo <- read_rds("indics_TERR_histodemo.rds" ) %>% ungroup() %>%
...
# remplacer par une fonction simple qui permet de ne pas importer migR
ajout_libelles_varventil_insee_s <-
  function(TABLE, VAR) {
    ref_var <- tribble(
      ~CODMOD, ~LIBMOD,
...
indics_migres_TERR_CS1 <-
  ajout_libelles_varventil_insee_s(TABLE = indics_migres_TERR_CS1 %>% ungroup(),
                                   VAR ="CS1")
...
# import carto
geo_TERR_poly <- st_read("geo_TERR_poly.gpkg", stringsAsFactors =F) %>% st_transform(4326) %>%
...
# retour de localisation à la racine
setwd("..")
               ...
               tabsetPanel(
                 tabPanel("Evolution démographique" ,
                          ...
                          column(7,  radioButtons("type_temporalite", "",
                                                  choices = list("Comparaison avec les autres territoires" = '2011_2016',
                                                                 "En historique depuis 1968" = '1968_2016'),
                                                  inline = FALSE,
                                                  width = "400px",
                                                  selected = '2011_2016'),
                                 ggiraphOutput(outputId="nuagepoint_evoldemo", width="600px",height="600px") %>%
                                   withSpinner(type = 1, color="#1B7F3F"))
                 ) ,
                 tabPanel("Arrivées et départs",
                          ...
                          column(7,ggiraphOutput(outputId="nuagepoint_terr", width="650px",height="700px") %>%
                                   withSpinner(type = 1, color="#1B7F3F")) ),
                 ...
                 tabPanel("Flux résidentiels", 
                          ...
                          radioButtons("type_flux", "Type de flux :",
                                       ...
                                       inline = TRUE,
                                       selected = 'ENTR') ,
                          column(9,ggiraphOutput(outputId="flux", width = 1200, height = 650) %>%
                                   withSpinner(type = 1, color="#1B7F3F")) )#,
               )
...
server <- function(input, output, session) {
  ...
  output$intro_onglet_arriveesdeparts <- renderUI({
    ...
    if(filteredData_TOT() %>% distinct(NIV_TERR) %>% pull() %in% 'DEP') {
      {HTML(paste0("<font size=2.5 color=#838587 family=Roboto>" ,
                   "Entre 2015 et 2016, quel est le bilan des échanges migratoires entre ", stri_trans_general(conv_accents(libelle_NIVGEO),"Latin-ASCII"),stri_trans_general(conv_accents(filteredData_nuagepoint_evoldemo() %>% filter
  ...
  output$pres_evoldemo <- renderUI({
    ...
    if(input$type_temporalite %in% '2011_2016') {
      ...
      {HTML(paste0("<br>",
                   ...
                   "<b>",paste0(symnum(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(EVOL_DEMO_TOT) %>% pull(),
                                       c(-Inf, 0, Inf),
                                       c("a diminué de ", "a augmenté de ")),
                                format(round(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(EVOL_DEMO_TOT) %>% pull() %>% abs(),-1), nsmall=0, big.mark=' '),""), " 
                   personnes", "</b>", "<br>",
                   "soit ",paste0(symnum(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_TOT) %>% pull(),
                                         c(-Inf,-0.01,-0.0025, 0.0025, 0.01, Inf),
                                         c(" une forte baisse ", " une baisse", " une relative stagnation", "une hausse", " une forte hausse")), ""),
                   
                   " de ", paste0(symnum(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_TOT) %>% pull(),
                                         c(-Inf, 0, Inf),
                                         c("", "+")),
                                  percent(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_TOT) %>% pull(), accuracy = 0.01),""), " en moyenne par 
                   an.<br>", "<br>",
                   "Cette évolution de la population résulte :", "<br>",
                   "- d'un solde naturel ",  paste0(symnum(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SN) %>% pull(),
                                                           c(-Inf,-0.01,-0.0025, 0.0025, 0.01, Inf),
                                                           c(" très négatif ", " négatif", " quasiment nul ", " positif", " fortement positif")), ""), 
                   " (",  paste0(symnum(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SN) %>% pull(),
                                        c(-Inf, 0, Inf),
                                        c("", "+")),
                                 percent(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SN) %>% pull(), accuracy = 0.01),""),
                   " par an) <br>",
                   
                   "- d'un solde migratoire * ",  paste0(symnum(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SMA) %>% pull(),
                                                                c(-Inf,-0.01,-0.0025, 0.0025, 0.01, Inf),
                                                                c(" très négatif ", " négatif", " quasiment nul ", " positif", " fortement positif")), ""), 
                   " (",  paste0(symnum(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SMA) %>% pull(),
                                        c(-Inf, 0, Inf),
                                        c("", "+")),
                                 percent(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SMA) %>% pull(), accuracy = 0.01),""),
    ...
    else if ( input$type_temporalite %in% '1968_2016') {
  ...
  output$nuagepoint_evoldemo <- renderggiraph({
    ...
    if(input$type_temporalite %in% '2011_2016') {
      ...
      borne_axe_max <-
        filteredData_nuagepoint_evoldemo() %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SMA) %>% max() %>%
        rbind(filteredData_nuagepoint_evoldemo() %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SMA) %>% min() %>% abs()) %>%
        rbind(filteredData_nuagepoint_evoldemo() %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SN) %>% max())  %>%
        rbind(filteredData_nuagepoint_evoldemo() %>% filter(periode %in% '2011_2016') %>% select(TX_EVOL_DEMO_AN_SN) %>% min() %>% abs()) %>% max()
      ...
      x <- girafe(ggobj = p1 , height_svg = 6)
      girafe_options(x, 
  ...
  output$pres <- renderUI({
    ...
    {HTML(paste0("<br>",
                 ...
                 stri_trans_general(conv_accents(libelle_NIVGEO),"Latin-ASCII"),"<b>",stri_trans_general(conv_accents(filteredData_nuagepoint_evoldemo() %>% filter(filtre_TERR %in% 'TERR') %>% distinct(LIB_TERR) %>% pull()),"Latin-
                                                                                                         ASCII"),"</b>","</font>" ,"<br>",
                 "<font size=2 color=#4f5154 family=Roboto> soit ",filteredData_TOT() %>% select(PE) %>% pull()  %>% percent(.,accuracy = 0.1)," de la population présente en 2016. </font> <br>",
                 "<br>",
                 ...
                 "<font size=2 color=#4f5154 family=Roboto>", "soit ",filteredData_TOT() %>% select(PS) %>% pull()  %>% percent(.,accuracy = 0.1)," de la population présente en 2015. </font> <br>",
                 "<br>",
  ...
  output$nuagepoint_terr <- renderggiraph({
    ...
    x <- girafe(ggobj = p1 , height_svg = 7)
    girafe_options(x, 
                   opts_tooltip(use_fill = FALSE, offx = 10, offy = -10, use_cursor_pos = TRUE, css = tooltip_css),
  ...
  filteredData_profilpop_CS1 <- reactive({
    if(input$maille_TERR %in% 'DEP') {
      indics_migres_TERR_CS1 %>%
        ...
        ajout_libelles_varventil_insee_s(TABLE = .,
                                         VAR ="CS1") %>%
  ...
  output$diagcirc_profil_pop_CS1 <- renderggiraph({
    ...
    gg_diagcirc_TERR <- ggplot(data = filteredData_profilpop_CS1() %>%
                                 ...
                                 mutate(tip = case_when(input$type_pop %in% 'PRES' ~ paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                                                            ...
                                                                                            "en 2016  ","sont des ","<b>",tolower(CS1_LIB),"</b>", "<br>"),
                                                        ...
                                                        input$type_pop %in% 'SORT' ~ paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                                                            ...
                                                                                            "entre 2015 et 2016 ","sont des ","<b>",tolower(CS1_LIB),"</b>", "<br>")
                                 ))
    ) +
    ...
    x <- girafe(code = print(ggdraw() +
                               draw_plot(gg_diagcirc_MOY + theme(legend.justification = "bottom"), x = 0.305, y = 0.435, width = 0.4, height = 0.3) +
  ...
  output$diagcirc_profil_pop_AGEREVS <- renderggiraph({
    ...
    gg_diagcirc_TERR = ggplot(data = filteredData_profilpop_AGEREVS() %>%
                                ...
                                mutate(tip = case_when(input$type_pop %in% 'PRES' ~ paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                                                           ...
                                                                                           "en 2016 ","ont ","<b>",AGEREVS_txt,"</b>", "<br>"),
                                                       input$type_pop %in% 'ENTR' ~ paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                                                           "<font size=2.5 color=white family=Roboto>",
                                                                                           "<b>", percent(value, accuracy = 2),"</b>"," des individus arriv&eacute;s dans ", 
                                                                                           stri_trans_general(conv_accents(libelle_NIVGEO),"Latin-ASCII"),"<b>",stri_trans_general(conv_accents(filteredData_nuagepoint_evoldemo() %>% 
                                                                                                                                                                                                  filter(filtre_TERR %in% 'TERR') %>% distinct(LIB_TERR) %>% pull()),"Latin-ASCII"),"</b>" ,"<br>",
                                                                                           "entre 2015 et 2016 ","ont ","<b>",AGEREVS_txt,"</b>", "<br>"),
                                                       input$type_pop %in% 'SORT' ~ paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                                                           "<font size=2.5 color=white family=Roboto>",
                                                                                           "<b>", percent(value, accuracy = 2),"</b>"," des individus ayant quitt&eacute; ",
                                                                                           stri_trans_general(conv_accents(libelle_NIVGEO),"Latin-ASCII"),"<b>",stri_trans_general(conv_accents(filteredData_nuagepoint_evoldemo() %>% 
    ...
    x <- girafe(code = print(ggdraw() +
                               draw_plot(gg_diagcirc_MOY + theme(legend.justification = "bottom"), x = 0.29, y = 0.4, width = 0.425, height = 0.325) +
                               draw_plot(gg_diagcirc_TERR + theme(legend.justification = "top"),  x =0, y = 0, width =  1, height =  1) ),
                #width = 10,
                height_svg = 7.2#,
                #height_svg = 5
    )
               ...
               mutate(tip = case_when( niveau_TERR %in% 'REG' ~
                                         paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                ...
                                                paste0(symnum(evol_pct_AUTO_PRES,
                                                       ...
                                                       round(evol_pct_AUTO_PRES*100, 2)," points de %)")," entre 2015 et 2016.", "<br>"),
                                       #" entre 2015 et 2016 par rapport aux autres r&eacute;gions.</font>", "<br>"  ),
                                       niveau_TERR %in% 'DEP' ~
                                         paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                ...
                                                paste0(symnum(evol_pct_AUTO_PRES,
                                                       ...
                                                       #percent(evol_pct_AUTO_PRES, accuracy = 0.01),""),
                                                       round(evol_pct_AUTO_PRES*100, 2)," points de %)")," entre 2015 et 2016.","<br>"),
                                       #" entre 2015 et 2016 par rapport aux autres d&eacute;partements.</font>", "<br>"  ),
                                       niveau_TERR %in% 'EPCI' ~
                                         paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
    ...
    if(input$type_flux %in% 'ENTR') {
      ...
      p_map_entr <-
        ...
        geom_segment_interactive(data=p.str.entr %>% 
                                   ...
                                   mutate(tip = case_when( niveau_TERR %in% 'REG' ~
                                                             paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                                    "<font size=2.5 color=white family=Roboto>", #"Entre 2015 et 2016,", "<br>",
                                                                    ...
                                                                    "entre 2015 et 2016, soit ", percent(pct_ind_ACTU_MIG, accuracy = 1), " des nouveaux arrivants." ),
                                                           niveau_TERR %in% 'DEP' ~
                                                             paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                                    "<font size=2.5 color=white family=Roboto>", #"Entre 2015 et 2016,","<br>",
                                                                    format(round(nb_ind,-1), nsmall=0, big.mark=" ")," personnes sont arriv&eacute;es ",#"<br>",
      ...
      tooltip_css <- "background-color:#272B30;padding:2px;font-size: 120%;color: white;opacity:0.2"
      x <- girafe(ggobj = p_map_entr , width_svg = 5)
      girafe_options(x, 
                     opts_tooltip(use_fill = FALSE, offx = 10, offy = -10, use_cursor_pos = TRUE, css = tooltip_css),
        ...
        geom_segment_interactive(data=p.str.sort %>% 
                                   ...
                                   mutate(tip = case_when( niveau_TERR %in% 'REG' ~
                                                             paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                                    ...
                                                                    "entre 2015 et 2016, soit ", percent(pct_ind_ANTE_MIG, accuracy = 1), " des partants."),
                                                           niveau_TERR %in% 'DEP' ~
                                                             paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                                                                    "<font size=2.5 color=white family=Roboto>", #"Entre 2015 et 2016,","<br>",
                                                                    format(round(nb_ind,-1), nsmall=0, big.mark=" ")," personnes ont quitt&eacute; ",#"<br>",
      ...
      tooltip_css <- "background-color:#272B30;padding:2px;font-size: 120%;color: white;opacity:0.2"
      x <- girafe(ggobj = p_map_sort , width_svg = 5)
      girafe_options(x, 
                     opts_tooltip(use_fill = FALSE, offx = 10, offy = -10, use_cursor_pos = TRUE, css = tooltip_css),
                     opts_hover(css = "stroke:red;r:2pt;"),
                     opts_selection(type = "none"),
      ...
      tooltip_css <- "background-color:#272B30;padding:2px;font-size: 120%;color: white;opacity:0.2"
      x <- girafe(ggobj = p_map_smnet ,width_svg = 5)
      girafe_options(x, 
                     opts_tooltip(use_fill = FALSE, offx = 10, offy = -10, use_cursor_pos = TRUE, css = tooltip_css),
